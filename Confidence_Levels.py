...
inv_y = -y_ear
squat_bottoms, properties = find_peaks(inv_y, distance=int(0.05 * frame_rate), prominence=prominence_value)
confidence_bottoms = properties['prominences']  # Confidence for bottoms

# Find squat tops (peaks)
squat_tops, properties = find_peaks(y_ear, distance=int(0.05 * frame_rate), prominence=prominence_value)
confidence_tops = properties['prominences']  # Confidence for tops
...
# Plot for visual check with confidence
plt.figure()
...
# Display confidence levels for squat starts and ends
for start, end, conf_start, conf_end in zip(squat_starts, squat_ends, confidence_tops, confidence_bottoms):
    t_mid = (time[start] + time[end]) / 2
    plt.text(t_mid, y_ear[start], f"Conf: {conf_start:.2f}", fontsize=8, color='green', ha='center')
    plt.text(t_mid, y_ear[end], f"Conf: {conf_end:.2f}", fontsize=8, color='black', ha='center')

# Vertical lines for each squat
...
plt.title("Squat Detection with Confidence Levels")
plt.legend()
plt.grid(True)
plt.show()
