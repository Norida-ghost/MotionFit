...
# Select ideal CSV file
ideal_file = filedialog.askopenfilename(
    title="Select the IDEAL CSV file",
    filetypes=[("CSV files", "*.csv")]
)
if not ideal_file:
    print("Ideal file not selected.")
    exit()

# Select folder with comparison files
compare_folder = filedialog.askdirectory(title="Select the folder with CSV files to compare")
if not compare_folder:
    print("No comparison folder selected.")
    exit()

# Get all CSV files from selected folder
file_paths = [
    os.path.join(compare_folder, f)
